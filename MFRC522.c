...
//  MSS_SPI_set_slave_select( &g_mss_spi1, MSS_SPI_SLAVE_0 );
  ...
  //активируем LOW level CS
  gpio_clear(GPIOA, GPIO4);
  delay_us(1);
  // Передаем адрес
  spi_send(SPI1, addr_bits);
...
//  MSS_SPI_set_slave_select( &g_mss_spi1, MSS_SPI_SLAVE_0 );
  ...
  // Передаем адрес с флагом чтения
  while(SPI_SR(SPI1) & SPI_SR_BSY){ }
  spi_send(SPI1, addr_bits);

  // Читаем данные из SPI
  rx_bits = spi_read(SPI1);
 ...
 * Return value: the successful return MI_OK
 */
u_char MFRC522_ToCard(u_char command, u_char *sendData, u_char sendLen, u_char *backData, uint8_t *backLen)
{
